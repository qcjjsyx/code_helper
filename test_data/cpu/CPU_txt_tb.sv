...
// 文件路径参数 - 根据地址选择不同的文件
parameter string FILE_PATH_1 = "/project/TPU2025/RTL/CPU/txt/rv64ui-p-lb.inst.txt";
parameter string FILE_PATH_2 = "/project/TPU2025/RTL/CPU/txt/rv64ui-p-lb.data.txt"; // 第二个文件路径
parameter ADDR_THRESHOLD = 56'h21200; // 地址阈值
...
reg [7:0] temp_char;
integer char_index;
reg use_file2; // 标志使用哪个文件
string current_file_path; // 当前使用的文件路径
...
.o_free2DDRWriteOver(o_free2DDRWriteOver)
  ...
  always @(posedge o_drv2DDRRead or negedge rst) begin
    if (!rst) begin
        ...
        // 根据地址选择文件
        if (o_readPA_56 < ADDR_THRESHOLD) begin
            // 地址小于阈值，使用第一个文件
            use_file2 = 0;
            current_file_path = FILE_PATH_1;
            // 计算行号（基于第一个文件的基地址0x1200）
            line_index = ((o_readPA_56 - 56'h1200) >> 5) + 1;
        end else begin
            // 地址大于等于阈值，使用第二个文件
            use_file2 = 1;
            current_file_path = FILE_PATH_2;
            // 计算行号（基于第二个文件的基地址，这里假设也是0x1200开始）
            // 如果第二个文件有不同基地址，需要调整这里
            line_index = ((o_readPA_56 - ADDR_THRESHOLD) >> 5) + 1;
        end
        
        $display("[%0t ns] DDR Read Request: Address=0x%h, Use File%s, Line Index=%0d", 
                 $time, o_readPA_56, (use_file2 ? "2" : "1"), line_index);
        
        // 打开文件并读取特定行
        file_handle = $fopen(current_file_path, "r");
        if (file_handle == 0) begin
            $display("Error: Cannot open file %s", current_file_path);
            i_ddrRefillLine_256 <= 256'h0;
        end else begin
            found = 0;
            line_counter = 0;
            ...
            while (!$feof(file_handle) && !found) begin
                ...
                if (line_counter == line_index) begin
                    ...
                    $display("[%0t ns] Successfully read from File%s line %0d: Data=%h", 
                             $time, (use_file2 ? "2" : "1"), line_index, i_ddrRefillLine_256);
                    found = 1;
                end else begin
                    // 使用简单的循环来跳过一行
                    char_index = 0;
            ...
            if (!found) begin
                $display("Error: Line %0d not found in File%s", line_index, (use_file2 ? "2" : "1"));
                i_ddrRefillLine_256 <= 256'h0;
            end
...
#50;
   ...
   #10;
   i_dataTPUtoCPU_80 = {16'h224,64'h1200};
   #5;
