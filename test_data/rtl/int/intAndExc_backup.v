...
(* dont_touch="true" *) wire [4:0] w_intType_5;//五种类型中断的存储
...
(* dont_touch="true" *) wire [19:0] w_allExcNum1_20;//走完中断判定分支后的中断码合�???
...
(* dont_touch="true" *) wire w_intnum_1;//用于提前判定是否有中断异�?
(* dont_touch="true" *) wire w_intnum_delay_1;//用于提前判定是否有中断异�?
(* dont_touch="true" *) wire w_intnum_neg_1;//下降沿检�?

assign {w_intPC_32,w_intType_5} = i_intPCAndIntNum_37;
...
assign w_intnum_1 = ((w_intType_5 == 0) & ({w_ifExcNum_4,w_decExcNum_4,w_exeExcNum_4} == 12'hfff)) ? 1 : 0;
delay4U intnumDelay (.inR(w_intnum_1), .outR(w_intnum_delay_1), .rst(rst));
assign w_intnum_neg_1 = ~w_intnum_1 & w_intnum_delay_1;
contTap firstTap(
.trig(w_intnum_neg_1 | w_SRDrive_1),
.req(o_intIsGo_1),
.rst(rst)
);
...
always @(posedge w_intSeleDrive_1 or negedge rst) begin
    ...
    else if (w_ifExc1Num_4 != 4'hf) r_exeNum_4 = w_ifExc1Num_4;//中断�??? 8/9/10/11/12
    else r_exeNum_4 = w_intNum1_4;
end

assign w_exeNum_4 = r_exeNum_4;

assign w_pc_32 = w_exeNum_4 == w_intNum1_4 ? w_intPC1_32 : //根据优先级判定结果选择带哪个中断下�???
...
assign w_exeAddr_32 = w_exeNum_4 * 4; // 需要修�???-->如果是异常返回需要变成从原来的栈中拿PC，访问Sarm的地址需要变�???
assign w_vecAddr_32 = (w_exeNum_4 == 4'b0010) ? (w_SP_32 + 24) : w_exeAddr_32;
...
(* dont_touch="true" *) cSplitter2_32b_int vectorSpli(.i_drive(w_DRSeleDriTovec_1), .i_data_32(w_pcAndPsr_64[63:32]), .o_free(w_freeFromDRSele_1),
...
begin
    if(!rst) begin 
      ...
      else if (w_exeNum_4 == 4'b0010) begin
        r_DRValid_2 = 2'b10;        
      end else begin
        r_DRValid_2 = 2'b01;        
      end
...
begin
  ...
  else if(w_exeNum_4 == 4'b1111) r_intAndExc_cnt = r_intAndExc_cnt;
  else if(w_exeNum_4 == 4'b0010) r_intAndExc_cnt = r_intAndExc_cnt - 1;
