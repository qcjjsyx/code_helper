...
//  delay8U rele6SplitterDelay(.inR(w_releSplitterDriveToRele6Splitter_1), .outR(w_releSplitterDriveToRele6SplitterDelay_1), .rst(rst));  
  //2024/11.14-->zlt ofree too slow
   (* dont_touch="true" *) cSplitter6_6b_launch rele6Splitter(.i_drive(w_releSplitterDriveToRele6Splitter_1), .i_data_6(w_rele1_6), .o_free(w_free),
...
//12/10 zwm if the inst is just has 1 rd,we should first store the r_preRdL the the r_preRdH
    ...
    delay6U driveFExcToIfDelay(.inR(i_driveFExcToIf_1), .outR(w_driveFExcToIf_1), .rst(rst));  
    ...
    assign w_regMergeDriveToRegSelector1_1 = w_driveFExcToIf_1 | w_regMergeDriveToRegSelector_1;
      always @(posedge w_regMergeDriveToRegSelector1_1 or negedge rst) begin
        if (!rst) begin
          ...
          if(w_isMultiLS_1 | w_load_1 & w_isLS_1 | i_isInInt & w_bx_1 | i_excToIfFlag_1) begin
            r_cont_2 <= 2'b0;
            r_isExeOne_1 <= 1'b0;
            r_isLsuOne_1 <= 1'b0;
          end else begin
...
// delay4U writeDelay0(.inR(w_writeRdFifoDrive_1), .outR(w_writeRdFifoDrive1_1), .rst(rst));
  ...
  assign w_pcComeFromWbFlag_1 = w_load_1 & (w_dHi_4 == 4'b1111 || w_registers_16[15] == 1'b1);
  delay8U pcDelay0(.inR(i_decoderDriveToLaunch_1), .outR(w_regSeleDri1ToPcsele_1), .rst(rst));
  delay6U pcDelay1(.inR(w_regSeleDri1ToPcsele_1), .outR(w_regSeleDri1ToPcsele1_1), .rst(rst));
...
//12/2 zwm change w_cond1_4 == 4'b1101 & w_z2_1 == 1 & w_n2_1 != w_v2_1 & w_isCb_1 == 1'b1 to w_cond1_4 == 4'b1101 & (w_z2_1 == 1 | w_n2_1 != w_v2_1) & w_isCb_1 == 1'b1
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // begin
        //   r_branchPc_32 = w_pc_32 + w_last2Imm_32 + $signed({1'b0,3'd4});
        // end
  ...
  assign o_branchPc_32 = r_branchPc_32;
  // wire w_bSeleDriToBFifo2_1;
  // delay16U b1Delay1(.inR(w_bSeleDriToBFifo1_1), .outR(w_bSeleDriToBFifo2_1), .rst(rst)); 
  (* dont_touch="true" *) cFifo1_32b_launch bFifo(.i_drive(w_bSeleDriToBFifo3_1), .i_data_32(), .o_free(w_bFifoFreeToBSele_1),
      .o_driveNext(o_bDriToIf), .o_data_32(), .i_freeNext(i_bFreeFromIf),
      .rst(rst));
