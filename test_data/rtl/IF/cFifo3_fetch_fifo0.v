...
wire w_outRRelay1_delay;
delay4U delay0_1 (.inR(w_outRRelay_4[1]), .outR(w_outRRelay1_delay), .rst(rst));

relay relay1_inst(
    .inR(w_outRRelay1_delay),
    .inA(w_outARelay_4[1]),
...
wire w_outRRelay2_delay;
delay8U delay1_2 (.inR(w_outRRelay_4[2]), .outR(w_outRRelay2_delay), .rst(rst));

relay relay2_inst(
    .inR(w_outRRelay2_delay),
    .inA(w_outARelay_4[2]),
...
delay2U outdelay0_inst (.inR(o_fire_3[2]), .outR(w_driveNext), .rst(rst));
delay2U outdelay1_inst (.inR(w_driveNext), .outR(o_driveNext), .rst(rst));
