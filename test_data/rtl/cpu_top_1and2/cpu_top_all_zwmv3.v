...
(* dont_touch="true" *) fetch fetch_inst (
    ...
    .o_drv2Int(w_intDriToIntTmp), .i_freeFInt(w_ifIntFreeFromIntTmp), .o_InterruptF_6(w_intNum_6), .i_interrupt(i_IntSig)
  );
//------------------------------------change end----------------------------------------------------------//
...
(* dont_touch="true" *) delay8U driveToDec1Delay(.inR(w_driveToDec1), .outR(w_driveToDec1Delay), .rst(rst)); 
assign w_launchToFetch_1 = w_intDriToIntTmp | w_driveToDec1Delay;//2/21 zwm change
...
assign w_lsuPyPathPmtFire_1 = w_lsuDriveToLaunch_1 | w_ExeDriveToLunchDelay_1 | w_branchDri3;//3/4 zwm add
always @(posedge w_lsuPyPathPmtFire_1 or negedge rst) begin
    if(!rst)begin
        r_releaseLsuByPathFlag_1 = 1'b1;
    end else begin
        if(w_lsuDriveToLaunch_1) begin
...
always @(posedge w_lsuDriveToWb2 or negedge rst) begin
    if (!rst) begin
        ...
        r_lsuToWBData_103[52:0] <= w_lsuToWriteBackData_103[52:0];
        r_lsuToWBData_103[102:53] <= w_lsuToWriteBackData_103[102:53];
...
(* dont_touch="true" *) intAndExc intAndExc_inst (
    .i_intDriveFromIF(w_intDriToInt),.i_intPCAndIntNum_38(w_ifInt_38),.o_intFreeToIF(w_ifIntFreeFromInt),
...
always @(posedge w_ifIntFire_1 or negedge rst) begin
    if(!rst)begin
        r_ifIntTmp_38 <= 38'h1ffff_fffc0;
    end else begin
        r_ifIntTmp_38 <= w_ifIntTmp_38;
    end
end
assign w_ifInt_38 = r_ifIntTmp_38;
