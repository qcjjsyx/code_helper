...
assign w_launchToFetch_1 = !(w_ExeDriveToLunch_1 | w_driveToDec | w_branchDri3);
...
(* dont_touch="true" *) delay8U ExeDriveToLunchDelay(.inR(w_ExeDriveToLunch_1), .outR(w_ExeDriveToLunch5_1), .rst(rst));
contTap exeTap2(
.trig(w_driveToDec | w_driveToDec5),
.req(w_driveToDecReq_1),
.rst(rst)
); 
(* dont_touch="true" *) delay8U ldriveToDecDelay(.inR(w_driveToDec), .outR(w_driveToDec5), .rst(rst));
...
always @(posedge w_launchToFetch5_1 or negedge rst) begin
    if(!rst)begin
        r_releaseExeByPathFlag_1 = 1'b1;
        r_count_1 = 1'b1;
    end else begin
