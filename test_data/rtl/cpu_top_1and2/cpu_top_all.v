...
//i_IntSig={iic,wd,spi1,uart1,timer}
input [5:0]i_IntSig, 
...
fetchSele1涓哄垽鏂槸鍚﹂渶瑕佸垎娲炬潵鐨勮烦杞琍C鎴栬€呬腑鏂紓甯哥粰鐨凱C锛寃_ExcToIfFlag_1鏍囧織鐫€璇C浠庝腑鏂紓甯告ā鍧楁潵鐨勶紝鐢变簬涓柇寮傚父妯″潡鏉ョ殑PC蹇呯�?
...
//---------------------------------------------------------------------------------------------------------//
/*鍙栨寚鍜岃瘧鐮佷箣闂寸殑绾ч棿瀵勫瓨鍣ㄣ€傝繖閲岀敤鎷╄矾鏉ュ疄鐜帮紝杩欓噷鐨勬嫨璺湁涓夌鎯呭喌锛氱涓€绉嶏紝濡傛灉鍙栨寚杩欓噷鏈変腑鏂垨鑰呭紓甯稿苟涓旇瘧鐮佸拰鎵ц閮芥病鏈夊紓甯革�?
閭ｄ箞灏辫蛋寮傚父鐨勬梺璺紱绗簩绉嶏紝濡傛灉璇戠爜鎴栨墽琛屾湁寮傚父锛岄偅涔堝氨璧伴槺锛涚涓夌锛屼笂闈袱绉嶉兘涓嶇鍚堬紝灏辫蛋姝ｅ父鐨勮繘鍏ヨ瘧鐮佺殑閭ｆ潯璺€傜壒鍒渶瑕佹敞鎰忕殑鏄�?
鍒ゆ柇璇戠爜鍜屾墽琛屾湁娌℃湁寮傚父鏄牴鎹紓甯哥爜鍒ゆ柇鐨勶紝杩欓噷鏄痺_decTrueNum_4,w_exeTrueNum_4锛屽墠闈㈡彁鍒拌瘧鐮佺殑寮傚父浜嬩欢鐢卞彇鎸囩粰鍑猴紝浣嗘槸杩欓噷鐨勫紓甯哥�?
...
//     .req(w_intCodeFlag_1),//1/13 zwm ------------->鐀��潵閫夋嫨鐀��殑涓柇鐮佹潵鑷摢閲�
//     .rst(rst)
//     );
...
assign w_iFExcOrIntFlag_1 = ({w_exceptionF_2_4,w_intExcNum_4,w_intNewType_6} != 14'b1111_1111_000000) && (w_exeTrueNum_4==4'hf); 
...
// (* dont_touch="true" *) delay4U outStackSeleDelay1(.inR(w_driveToDec), .outR(w_driveToDec3), .rst(rst));//2/25 zwm change
(* dont_touch="true" *) delay8U outStackSeleDelay0(.inR(w_driveToDec), .outR(w_driveToDec3), .rst(rst));

(* dont_touch="true" *) delay4U outStackSeleDelay1(.inR(w_driveToDec3), .outR(w_driveToDec1), .rst(rst));
...
鎵€浠ュ繀椤诲府鎵ц鍙戝紓甯镐簨浠躲€俧ifo杩欓噷鐨刬_freeNext鐗瑰埆閲嶈锛寃_launchDriveToExe11鏄墽琛屾敹鍒板垎娲剧殑浜嬩欢骞朵笖寤惰繜涓€娈垫椂闂村悗鐨勶紝鎵€浠ユ鏃跺氨蹇呯�?
...
wire w_bitOpOver_1,w_bitOpOver1_1;
...
(* dont_touch="true" *) delay8U goDelay6(.inR(w_bitOpOver_1), .outR(w_bitOpOver1_1), .rst(rst)); 
wire w_goDrive;
wire w_lsuDriveToWriteBack_1, w_multiLoadOrStoreOverData_1;


wire w_exeDrive_1, w_exeDrive1_1,w_freeToExeMer_1, w_exeReq,w_wbreq;
...
contTap thirdTap(
.trig(w_bitOpOver_1 | w_bitOpOver1_1),
.req(w_wbreq),
.rst(rst)
);
...
//11/8 zwm
assign w_goDrive = w_decoderDrive2_1 | w_muquitltiLoadOrStoreOver1_1 | w_loadEndDrive1 | w_bitOpOver1_1;
...
//3/4 zwm -> add bit operation
        if(w_multiLSEnd_1 & w_multiLoadOrStoreOverData_1 | w_exeReq & w_loadEndFlag | w_wbreq)begin
            r_isGo = 1'b1;
        end
        else if(w_decoData_187[5] | w_decoData_187[57] & w_decoData_187[53] | w_decoData_187[89])begin
            r_isGo = 1'b0;
        end
    end
end
...
//---------------------------------------------------------------------------------------------------------//
/*鍒嗘淳瀵逛笌鏃佽矾妫€娴嬭鏁板櫒鏇存柊鐨勯棶棰樸€傚彧瑕佸彂鐢熶簡涓柇寮傚父锛岄偅涔堜笉绠℃€庝箞鏍凤紝鍦ㄥ綋鍓嶆祦姘寸嚎琚啿鍒蜂箣鍚庯紝杩涙潵鐨勬寚浠ゅ繀鐒舵槸绗竴鏉℃寚浠わ紝鎵€鏈夌�?
鏉′欢閮芥槸鍒濆鐨勬潯浠讹紝鎵€浠ユ瘡褰撲腑鏂紓甯告ā鍧楀線鍙栨寚鍙戜簨浠舵椂锛屾剰鍛崇潃瑕佽繘鏉ヤ腑鏂湇鍔＄▼搴忔垨鑰呭紓甯歌繑鍥炵殑绗竴鏉℃寚浠や簡锛屾鏃堕兘瑕佹妸鎵€鏈夌殑鏉′欢鍒濆鍖�?
杩欓噷i_driveFExcToIf_1鍜宨_excToIfFlag_1灏辨槸鐢ㄤ簬鎶婂垎娲捐鏁板櫒鍒濆鍖栫�?
*/
//---------------------------------------------------------------------------------------------------------//  
...
/*
1/12 鏇存�?
涓嶉渶瑕佺敤鍒拌繖涓爣蹇椾綅浜嗭紝鍥犱负鏃犳硶鎶婃彙杩欎釜鏃舵満锛屽鏋�?+1鏉℃寚浠よ繕娌¤繘璇戠爜锛屼絾鏄痭鏉℃寚浠ゅ凡缁忚繘鎵ц骞朵笖鏃佽矾鍙戝嚭鏉ヤ簡锛岃繖涓椂鍊欐墽琛岀殑鏃佽矾鏄�
闇€瑕佺殑锛屽彧涓嶈繃鏄�?+1鏉℃寚浠よ繕娌¤繘鍒拌瘧鐮佸垎娲�
*/
//---------------------------------------------------------------------------------------------------------//  
...
//---------------------------------------------------------------------------------------------------------//
/*杩欓噷鏄垎娲惧拰鎵ц涔嬮棿鐨勭骇闂村瘎瀛樺櫒銆傝繖閲岀殑鎯呭喌鏈€澶嶆潅锛屽洜涓烘嫨璺墠闈㈣繕鏈変竴涓簰鏂ワ紝褰撴嫨璺殑浜嬩欢鏄粠鍒嗘淳杩囨潵鐨勶紝鎷╄矾鏈変笁绉嶆儏鍐碉�?
...
//----------------------------------------------------------------------------//
            if(r_isGo == 1'b0 && 
            (r_launchDataToExe_207[187] |
             r_launchDataToExe_207[134] & r_launchDataToExe_207[130] |
             r_launchDataToExe_207[201] | r_launchDataToExe_207[200] | r_launchDataToExe_207[199] | r_launchDataToExe_207[198])) r_lsuGo_1 <= 1'b0; // 2024.11.13--zlt-->lsuWay
//----------------------------------------------------------------------------//
            else r_lsuGo_1 <= 1'b1;
        end
    end     
...
wire w_exeByPathDriveToLaunchDelay_1,w_exeByPathDriveToLaunchDelay2_1;
(* dont_touch="true" *) delay8U exeToLaunhDelay(.inR(w_exeByPathDriveToLaunch_1), .outR(w_exeByPathDriveToLaunchDelay_1), .rst(rst)); 
(* dont_touch="true" *) delay8U exeToLaunhDelay2(.inR(w_exeByPathDriveToLaunchDelay_1), .outR(w_exeByPathDriveToLaunchDelay2_1), .rst(rst)); 

always @(posedge w_exeByPathDriveToLaunchDelay2_1 or negedge rst) begin
    if (!rst) begin
        r_exeToLaunchData_96 <= 96'b0; 
        // r_executeDataToLsu_163 <= 163'b0; 
        // r_wen2_2 <= 2'b0;
...
鏃╁凡鍙戝嚭鏉ワ紝鏃犳硶琚秷闄わ紝鎵€浠ュ繀椤讳繚璇佷笅涓€鏉℃寚浠ゅ彂鍒拌瘧鐮佷箣鍚庢墠鎶婃墽琛屾梺璺彂鍑烘�?
*/
//--------------------------------------------------------------------------------------------------------------------//
wire w_releaseExeByPathFlag_1;
reg r_releaseExeByPathFlag_1;
wire w_ExeDriveToLunchDelay_1,w_ExeDriveToLunchDelay2_1;
...
wire w_driveToDec1Delay;
wire w_intDriToIntTmpDelay;
wire w_intReq;
(* dont_touch="true" *) delay16U driveToDec1Delay(.inR(w_driveToDec1), .outR(w_driveToDec1Delay), .rst(rst)); //3/14 zwm change 8U to 16U
(* dont_touch="true" *) delay8U intDriToIntTmpDelay(.inR(w_intDriToIntTmp), .outR(w_intDriToIntTmpDelay), .rst(rst)); 
contTap intTmpTap(
.trig(w_intDriToIntTmp | w_intDriToIntTmpDelay),
.req(w_intReq),
.rst(rst)
);
//--------------------------------------------------------------------------------------------------------------------//
assign w_launchToFetch_1 = w_intDriToIntTmpDelay | w_driveToDec1Delay;//2/21 zwm change
...
(* dont_touch="true" *) cPmtFifo1 exeByPathPmtfifo(.i_drive(w_ExeDriveToLunch_1), .o_free(w_launchFreeToExe1_1), .pmt(w_releaseExeByPathFlag_1),
.o_driveNext(w_ExeDriveToLunchDelay_1), .i_freeNext(w_launchFreeToExe_1), .o_fire_1(w_exeByPathFire_1),
.rst(rst));
...
//         r_releaseExeByPathFlag_1 = 1'b1;
//     end else begin
//         if( w_launchDriveToIf_1 | w_branchDri1) begin
//             r_releaseExeByPathFlag_1 = 1'b0;
//         end else begin
//             r_releaseExeByPathFlag_1 = 1'b1;
//         end
//     end
...
// // (* dont_touch="true" *) delay6U exeByPathDelay(.inR(w_ExeDriveToLunch_1), .outR(w_ExeDriveToLunchDelay_1), .rst(rst)); 
// (* dont_touch="true" *) cPmtFifo1 exeByPathPmtfifo(.i_drive(w_ExeDriveToLunch_1), .o_free(w_launchFreeToExe1_1), .pmt(w_releaseExeByPathFlag_1),
// .o_driveNext(w_ExeDriveToLunchDelay_1), .i_freeNext(w_launchFreeToExe_1), .o_fire_1(),
// .rst(rst));

//-------------------------------change end-------------------------------------------------------------//
// 瑜版挻娈忛崑婊勭ウ濮樺鍤庨弮鑸靛⒔鐞涘瞼娈戦弮浣界熅闂団偓鐟曚浇鍨楀鍐╁竴
...
杩欎袱绉嶆儏鍐甸兘涓嶅簲璇ュ線鍒嗘淳鍙戞梺璺紝鍥犱负鍒嗘淳鍚庨潰涓嶄細鍐嶆湁鏂扮殑鎸囦�?
*/
//---------------------------------------------------------------------------------------------------------//  
...
(* dont_touch="true" *) cSelector2_1b launchWaySele(.i_drive(w_ExeDriveToLunchDelay_1),//1/27 zwm ------------->change w_ExeDriveToLunch_1 to w_ExeDriveToLunchDelay_1
     .i_data_1(w_launchWaySeleCS_1), .o_free(w_launchFreeToExe_1),
...
鎵ц妯″潡鏉ワ紝閭ｄ箞鍙渶瑕佸垽鏂墽琛屾湁娌℃湁寮傚父灏卞彲浠ワ紝濡傛灉鏈夊氨璧板紓甯告梺璺紝娌℃湁灏辨甯稿幓寰€璁垮瓨锛涚浜岀鏄粠涓婁竴涓骇闂存嫨璺殑寮傚父鏃佽矾鏉ワ�?
*/
//---------------------------------------------------------------------------------------------------------//  
//1/3 zwm 
...
wire w_lsuFreeFromLaunchDelay_1;
...
(* dont_touch="true" *) lsu lsu_inst (
    ...
    .o_lsuDriveToLaunch_1(w_lsuDriveToLaunch_1), .o_lsuToLaunchData_64(w_lsuToLaunchData_64), .i_lsuFreeFromLaunch_1(w_lsuFreeFromLaunchDelay_1),
...
//3/4 zwm -> due to may 
  wire w_lsuDriveToLaunchDelay_1;
  cFifo1 lsuDriveToLaunchDelayFifo1(.i_drive(w_lsuDriveToLaunch_1), .i_freeNext(w_lsuFreeFromLaunch_1), .rst(rst),
  .o_free(w_lsuFreeFromLaunchDelay_1), .o_driveNext(w_lsuDriveToLaunchDelay_1), .o_fire_1());


//-----------------------------------------------big change-----------------------------------------------//
//12/24 zwm add a arbitMerge
  wire w_lsuDriveToIcacheDelay_1,w_fetchDriveToIcacheDelay_1;
  (* dont_touch="true" *) delay8U lsuToIcacheDelay(.inR(w_lsuDriveToIcache_1), .outR(w_lsuDriveToIcacheDelay_1), .rst(rst));//3/14 zwm 4U->8U
...
//2/11 zwm add delay6U
  ...
  wire [64:0] w_inst_65;
  reg [64:0] r_inst_65;
  always@(posedge i_drvFICache or negedge rst)begin
    if(!rst)begin
        r_inst_65 <= 65'b0;
    end else begin
        r_inst_65 <= i_inst_65;
  ...
  cSelector2_65b_cpu icacheSelector(
    .i_drive(w_drvFICacheDelay1), .i_data_65(w_inst_65), .o_free(o_free2ICache),
...
wire w_lsuDriveToLaunch3_1;
wire w_lsuDriveToLaunchDelay2_1;
...
(* dont_touch="true" *) delay6U lsuDelay1(.inR(w_lsuDriveToLaunch_1), .outR(w_lsuDriveToLaunch3_1), .rst(rst)); 
(* dont_touch="true" *) delay4U lsuDelay2(.inR(w_lsuDriveToLaunch3_1), .outR(w_lsuDriveToLaunch2_1), .rst(rst)); 
...
(* dont_touch="true" *) delay4U lsuDelay0(.inR(w_lsuDriveToLaunchDelay2_1), .outR(w_lsuDriveToLaunch1_1), .rst(rst)); 

(* dont_touch="true" *) cSplitter2_1b lsuSplitter(.i_drive(w_lsuDriveToLaunch1_1), .i_data_1(1'b0), .o_free(w_lsuFreeFromLaunch_1),
    .o_driveNext0(w_LsuDriveToLunch1_1), .i_freeNext0(w_launchFreeToLsu2_1), .o_data0_1(),
...
wire w_lsuPyPathPmtFire_1;
wire w_w_lsuDriveToLaunchReq_1;
(* dont_touch="true" *) delay8U lsuDriveToLaunchDelay0(.inR(w_lsuDriveToLaunch_1), .outR(w_lsuDriveToLaunchDelay2_1), .rst(rst)); 
contTap lsuDriveToLaunchTap(
.trig(w_lsuDriveToLaunch_1 | w_lsuDriveToLaunchDelay2_1),
.req(w_w_lsuDriveToLaunchReq_1),
.rst(rst)
);
assign w_lsuPyPathPmtFire_1 = w_lsuDriveToLaunchDelay2_1 | w_ExeDriveToLunchDelay_1 | w_branchDri3;//3/4 zwm add
always @(posedge w_lsuPyPathPmtFire_1 or negedge rst) begin
    if(!rst)begin
        r_releaseLsuByPathFlag_1 = 1'b1;
    end else begin
        if(w_w_lsuDriveToLaunchReq_1) begin
            r_releaseLsuByPathFlag_1 = 1'b0;
        end else begin
            r_releaseLsuByPathFlag_1 = 1'b1;
        end
    end
...
// (* dont_touch="true" *) delay6U exeByPathDelay(.inR(w_ExeDriveToLunch_1), .outR(w_ExeDriveToLunchDelay_1), .rst(rst)); 
(* dont_touch="true" *) cPmtFifo1 lsuByPathPmtfifo(.i_drive(w_LsuDriveToLunch1_1), .o_free(w_launchFreeToLsu2_1), .pmt(w_releaseLsuByPathFlag_1),
...
assign w_lsuByWaySele0CS_1 = r_lsuGo_1 && w_decoderOutStackSeleFlag_2==2'b01 && w_ifOutStackSeleFlag_2==2'b01;//1/12 zwm ---->use this
      (* dont_touch="true" *) cSelector2_1b lsuByWaySele0(.i_drive(w_LsuDriveToLunchDelay_1), .i_data_1(w_lsuByWaySele0CS_1), .o_free(w_launchFreeToLsu1_1),
...
wire w_rele0driveToMe_1,w_rele0driveToMe1_1;
  ...
  .rst(rst)); // exe     // 閿熸枻鎷烽敓鏂ゆ嫹閿熸枻鎷蜂竴閿熸枻鎷锋病閿熸枻鎷锋墽閿熷彨纰夋嫹閿熸枻鎷疯�?
...
wire w_fire9_1,w_fire10_1,w_fire11_1,w_fire12_1,w_fire13_1,w_fire14_1,w_fire15_1;
wire w_lsuDriveToWb2Buff_1;
BUFM2HM buf8(.A(w_lsuDriveToWb2), .Z(w_fire9_1));
BUFM2HM buf9(.A(w_fire9_1), .Z(w_fire10_1));
BUFM2HM buf10(.A(w_fire10_1), .Z(w_fire11_1));
BUFM2HM buf11(.A(w_fire11_1), .Z(w_fire12_1));
BUFM2HM buf12(.A(w_fire12_1), .Z(w_fire13_1));
BUFM2HM buf13(.A(w_fire13_1), .Z(w_fire14_1));
BUFM2HM buf14(.A(w_fire14_1), .Z(w_fire15_1));
BUFM2HM buf15(.A(w_fire15_1), .Z(w_lsuDriveToWb2Buff_1));

always @(posedge w_lsuDriveToWb2 or negedge rst) begin
    if (!rst) begin
        r_lsuToWBData_103 <= 103'b0;
        r_nzcvWen3_4 <= 4'b0; 
    end else begin
        r_lsuToWBData_103[31:0] <= w_lsuToWriteBackData_103[31:0];
        r_lsuToWBData_103[63:32] <= w_lsuToWriteBackData_103[63:32];
        r_lsuToWBData_103[95:64] <= w_lsuToWriteBackData_103[95:64];
        r_lsuToWBData_103[102:96] <= w_lsuToWriteBackData_103[102:96];
...
(* dont_touch="true" *) grf grf_inst (
    ...
    .o_grfDriveToWb_1(w_grfDriveToWb_1), .o_grfDataToWb_64(w_grfDataToWb_64), .i_grfFreeFromWb_1(w_grfFreeFromWb_1), //WB閿燂�???????????
    ...
    .o_grfwFreeToWB_1(w_grfFreeToWB_1), //WB閿燂�??????????
    
    .i_driveFromExp_1(w_driveToRGRF_1), .o_grfFreeToExp_1(w_freeFromRGRF_1),
    .o_grfDriveToExp_1(w_driveFromRGRF_1), .o_grfDataToExp_192(w_grfData_192), .i_grfFreeFromExp_1(w_freeToRGRF_1), //exp閿燂�???????????
    ...
    .o_grfwFreeToExp_1(w_freeFromWGRF_1) //exp閿燂�??????????
  );
